...
plot_UMAP <- function(df_time = NULL, df_freq = NULL, label_vec = NULL) {
  if (is.null(df_time)) {
    stop("Provide valid df_time.")
  }

  if (is.null(df_freq)) {
    stop("Provide valid df_freq.")
  }

  if (is.null(label_vec)) {
    stop("Provide valid label_vec.")
  }

  if (nrow(df_time) != nrow(df_freq)) {
    stop("Number of rows of df_time should be the same as number of rows of df_freq.")
  }

  if (nrow(df_time) != length(label_vec)) {
    stop("Number of rows of df_time should be the same as length of label_vec.")
  }
    ...
    server = function(input, output, session) {
      ...
      output$UMAP_feature <- shiny::renderPlot({
        ...
        ggplot2::ggplot(umap_df1, ggplot2::aes(x = V1, y = V2, color = feature,
          ...
          ggplot2::scale_color_gradientn(colours = rcartocolor::carto_pal(5, "TealGrn"),
                                         values = values_col) +
          ggplot2:: theme(axis.text.x = ggplot2::element_text(size = 12, face = "bold"),
                          axis.text.y = ggplot2::element_text(size = 12, face = "bold"),
                          axis.title.x = ggplot2::element_text(size = 14, face = "bold"),
